(function () {
    function day_7(puzzle) {
        ...
        const steps = puzzle.split('\n').map(x => x.match(/[A-Z]/g)).reduce((acc, curr) => {
            ...
            if (!acc[beforeStep])
                acc[beforeStep] = { letter: beforeStep, dependsOn: [], seconds: beforeStep.charCodeAt(0) - minSeconds };

            if (acc[step])
                acc[step].dependsOn.push(beforeStep);
            else
                acc[step] = { letter: step, dependsOn: [beforeStep], seconds: step.charCodeAt(0) - minSeconds };
            return acc;
        }, {});

        let starters = Object.values(steps).filter(x => !x.dependsOn.length).map(x => x.letter);
        let answer1 = '';
        let answer2 = 0;
