(function () {
    ...
    function naughtOrNice1(string) {
        ...
        for (var j = 0; j < string.length; j++) {
            ...
            if (lastChar === char)
                duplicate = true;
            if (~avoid.indexOf(lastChar + char))
                avoided = false;
            lastChar = char;
        }
    ...
    function naughtOrNice2(string) {
        ...
        for (var j = 0, l = string.length; j < l; j++) {
            if (j < l - 1 && (string[j] + string[j + 1] !== string[j + 1] + string[j + 2] || string[j] === string[j + 3])) {
    ...
    function day_5(puzzle) {
        ...
        for (var i = 0; i < puzzle.length; i++) {
            var string = puzzle[i];
            if (naughtOrNice1(string))
                answer1++;
            if (naughtOrNice2(string))
                answer2++;
        }
    ...
    December.addDay({
        ...
        questions: ['How many strings are nice?', 'How many strings are nice under these new rules?'],
        answer: day_5,
        example: function () {
