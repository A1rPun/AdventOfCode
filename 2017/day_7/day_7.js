(function () {
        ...
        function findUnbalance(obj) {
            ...
            for (var key in obj) {
                ...
                if (balance && weight !== balance)
                    answer2 = 'name:' + key + ' balance:' + balance + ' weight:' + weight + ' self:' + lookup[key].weight + ' answer:' + (lookup[key].weight-7);
                else
                    balance = weight;
    ...
    December.addDay({
        ...
        questions: ['What is the name of the bottom program?', 'Given that exactly one program is the wrong weight, what would its weight need to be to balance the entire tower?'],
        answer: day_7,
        example: function () {
            return `pbga (66)
xhth (57)
