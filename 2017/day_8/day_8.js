(function () {
    ...
    function day_8(puzzle) {
        ...
        for (let i = 0; i < puzzle.length; i++) {
            ...
            if (checkCondition(register[condition[0]] || 0, condition[1], +condition[2])) {
                var modification = instruction[0].split(' ');
                var currentValue = register[modification[0]] || 0;
                currentValue = modification[1] === 'inc'
                    ? currentValue + +modification[2]
                    : currentValue - +modification[2];
                if (currentValue > answer2)
        ...
        for (var key in register) {
            if (register[key] > answer1)
                answer1 = register[key];
        }
